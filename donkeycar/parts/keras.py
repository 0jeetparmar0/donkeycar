...
def core_cnn_layers(img_in, drop, l4_stride=1):
    ...
    """
    x = img_in
    x = conv2d(24, 5, 2, 1, activation='elu' )(x)
    x = Dropout(drop)(x)
    x = conv2d(36, 5, 2, 2, activation='elu')(x)
    x = Dropout(drop)(x)
    x = conv2d(48, 5, 2, 3,activation='elu')(x)
    x = Dropout(drop)(x)
    x = conv2d(64, 3,  l4_stride,4,activation='elu')(x)
    x = Dropout(drop)(x)
    x = conv2d(64, 3, 1, 5,activation='elu')(x)
    x = Dropout(drop)(x)
...
def default_n_linear(num_outputs, input_shape=(120, 160, 3)):
    ...
    x = Dense(1164, activation='elu', name='dense_1')(x)
    x = Dropout(drop)(x)
    x = Dense(100, activation='elu', name='dense_2')(x)
    x = Dropout(drop)(x)
    x = Dense(50, activation='elu', name='dense_3')(x)
    x = Dropout(drop)(x)
    x = Dense(10, activation='elu', name='dense_4')(x)
    x = Dropout(drop)(x)
    outputs = []
